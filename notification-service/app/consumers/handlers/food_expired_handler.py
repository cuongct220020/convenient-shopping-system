# notification-service/app/consumers/handlers/food_expired_handler.py
...
from app.templates.notification_templates import FoodExpiredNotificationTemplate
...
logger = get_logger("Food Expired Handler")


class FoodExpiredHandler(BaseMessageHandler):
    async def handle(self, message: dict, app=None):
        """
        Handle the FOOD_EXPIRED notification.
        ...
        try:
            rendered = FoodExpiredNotificationTemplate.render(raw_data)
            ...
            async with postgres_db.get_session() as session:
                ...
                for r in receivers or []:
                    ...
                    created = await repo.create(NotificationCreateSchema(
                        ...
                        template_code=FoodExpiredNotificationTemplate.template_code,
                    ...
                    logger.info(
                        ...
                        f"template={FoodExpiredNotificationTemplate.template_code} "
