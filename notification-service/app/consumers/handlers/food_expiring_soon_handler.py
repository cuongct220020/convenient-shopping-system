# notification-service/app/consumers/handlers/food_expiring_soon_handler.py
...
from app.templates.notification_templates import FoodExpiringSoonNotificationTemplate
...
logger = get_logger("Food Expiring Soon Handler")


class FoodExpiringSoonHandler(BaseMessageHandler):
    async def handle(self, message: dict, app=None):
        """
        Handle the FOOD_EXPIRING_SOON notification.
        ...
        try:
            rendered = FoodExpiringSoonNotificationTemplate.render(raw_data)
            ...
            async with postgres_db.get_session() as session:
                ...
                for r in receivers or []:
                    ...
                    created = await repo.create(NotificationCreateSchema(
                        ...
                        template_code=FoodExpiringSoonNotificationTemplate.template_code,
                    ...
                    logger.info(
                        ...
                        f"template={FoodExpiringSoonNotificationTemplate.template_code} "
