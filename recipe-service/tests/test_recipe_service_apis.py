...
def setup_auth():
    print_info("--- Setting up Admin Auth ---")
    # Default credentials for local/dev env
    # Ensure you have run: python3 user-service/scripts/create_admin_user.py
    username = "admin"
    password = "admin" 
    ...
    if resp.status_code == 200:
        ...
        print_info(f"Access Token: {token[:20]}...") # Print first 20 chars for safety
        context["headers"] = {"Authorization": f"Bearer {token}"}
        print_success("Logged in successfully as Admin")
        return True
    else:
        print_error(f"Login failed: {resp.text}")
        print_info("HINT: Did you run 'python3 user-service/scripts/create_admin_user.py' to create the admin user?")
        return False
...
def test_create_ingredients():
    ...
    for ing in ingredients:
        try:
            resp = requests.post(f"{BASE_URL}/v2/ingredients/", json=ing, headers=context["headers"], verify=VERIFY_SSL)
            if resp.status_code == 201:
                ...
                search_resp = requests.get(f"{BASE_URL}/v2/ingredients/search", params={"keyword": ing['component_name']}, headers=context["headers"], verify=VERIFY_SSL)
...
def test_get_search_filter_ingredients():
    ...
    if context["ingredient_ids"]:
        ...
        resp = requests.get(f"{BASE_URL}/v2/ingredients/{first_id}", headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 200:
    ...
    # Search
    resp = requests.get(f"{BASE_URL}/v2/ingredients/search", params={"keyword": "thịt"}, headers=context["headers"], verify=VERIFY_SSL)
    if resp.status_code == 200:
        data = resp.json()['data']
        if len(data) > 0:
    ...
    # Filter
    resp = requests.get(f"{BASE_URL}/v2/ingredients/filter", params={"category": "Rau củ"}, headers=context["headers"], verify=VERIFY_SSL)
    if resp.status_code == 200:
        data = resp.json()['data']
        if len(data) > 0:
...
def test_create_recipes():
    ...
    for r in recipes:
        ...
        resp = requests.post(f"{BASE_URL}/v2/recipes/", json=r, headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 201:
            data = resp.json()
            print_success(f"Created {data['component_name']} (ID: {data['component_id']})")
            context["recipe_ids"][data["component_name"]] = data["component_id"]
        elif resp.status_code == 400 and ("unique constraint" in resp.text.lower() or "uniqueviolation" in resp.text.lower()): # or similar
             # Try search
             print_info(f"Recipe {r['component_name']} might exist, searching...")
             search_resp = requests.get(f"{BASE_URL}/v2/recipes/search", params={"keyword": r['component_name']}, headers=context["headers"], verify=VERIFY_SSL)
             if search_resp.status_code == 200:
    ...
    if thit_kho_id:
        ...
        resp = requests.post(f"{BASE_URL}/v2/recipes/", json=com_chien, headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 201:
            data = resp.json()
            ...
            context["recipe_ids"][data["component_name"]] = data["component_id"]
             ...
             search_resp = requests.get(f"{BASE_URL}/v2/recipes/search", params={"keyword": "Cơm chiên"}, headers=context["headers"], verify=VERIFY_SSL)
             if search_resp.status_code == 200:
                 for res in search_resp.json()['data']:
...
def test_get_search_recipes():
    ...
    resp = requests.get(f"{BASE_URL}/v2/recipes/{r_id}", headers=context["headers"], verify=VERIFY_SSL)
    if resp.status_code == 200:
    ...
    if com_chien_id:
        resp = requests.get(f"{BASE_URL}/v2/recipes/detailed/{com_chien_id}", headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 200:
    ...
    # Search
    resp = requests.get(f"{BASE_URL}/v2/recipes/search", params={"keyword": "thịt"}, headers=context["headers"], verify=VERIFY_SSL)
    if resp.status_code == 200:
...
def test_flattened():
    ...
    if salad_id and thit_id:
        ...
        resp = requests.post(f"{BASE_URL}/v2/recipes/flattened", params={"check_existence": "false"}, json=payload, headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 200:
...
def test_update():
    ...
    if ing_id:
        ...
        resp = requests.put(f"{BASE_URL}/v2/ingredients/{ing_id}", json={"type": "countable_ingredient", "component_name": new_name}, headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 200:
            print_success(f"Update Ingredient {ing_id} to {new_name}: OK")
            # Revert
            requests.put(f"{BASE_URL}/v2/ingredients/{ing_id}", json={"type": "countable_ingredient", "component_name": ing_name}, headers=context["headers"], verify=VERIFY_SSL)
        else:
    ...
    if rec_id:
        resp = requests.put(f"{BASE_URL}/v2/recipes/{rec_id}", json={"default_servings": 5}, headers=context["headers"], verify=VERIFY_SSL)
...
def test_get_all():
    print_info("\n--- 7. GET ALL - Ingredients/Recipes ---")
    
    # Get all ingredients
    resp = requests.get(f"{BASE_URL}/v2/ingredients/", params={"limit": 5}, headers=context["headers"], verify=VERIFY_SSL)
    if resp.status_code == 200:
        data = resp.json()
        print_success(f"Get All Ingredients: Found {len(data['data'])} items (Page size: {data['size']})")
    else:
        print_error(f"Get All Ingredients failed: {resp.text}")

    # Get all recipes
    resp = requests.get(f"{BASE_URL}/v2/recipes/", params={"limit": 5}, headers=context["headers"], verify=VERIFY_SSL)
    if resp.status_code == 200:
        data = resp.json()
        print_success(f"Get All Recipes: Found {len(data['data'])} items (Page size: {data['size']})")
    else:
        print_error(f"Get All Recipes failed: {resp.text}")

# def test_recommend():
#     print_info("\n--- 8. RECOMMEND - Recipes ---")
#     # Need a group ID. Let's create a temporary group via User Service
...
#         print_info(f"Created temporary group: {group_id}")
#
#         # Test recommend
#         resp = requests.get(f"{BASE_URL}/v2/recipes/recommend", params={"group_id": group_id}, headers=context["headers"], verify=VERIFY_SSL)
#         if resp.status_code == 200:
#             print_success(f"Recommend Recipes: OK (Found {len(resp.json())} items)")
#         else:
#             print_error(f"Recommend Recipes failed: {resp.text}")
#
#         # Cleanup group (optional, but good practice)
#         requests.delete(f"{BASE_URL}/api/v1/user-service/groups/{group_id}", headers=context["headers"], verify=VERIFY_SSL)
#     else:
#         print_error(f"Failed to create group for recommend test: {resp.text}")

def test_delete():
    print_info("\n--- 9. DELETE - Ingredients/Recipes ---")
    
    # Delete Ingredient - Use an ingredient NOT used in any recipe to avoid FK violation
    # "Trứng gà" is used in "Cơm chiên" and "Bữa sáng đầy đủ"
    # "Chuối" is created but not used in any recipe in this script
    ing_name = "Chuối"
    ing_id = context["ingredient_ids"].get(ing_name)
    
    if ing_id:
        print_info(f"Attempting to delete unused ingredient: {ing_name} (ID: {ing_id})")
        resp = requests.delete(f"{BASE_URL}/v2/ingredients/{ing_id}", headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 204:
            print_success(f"Delete Ingredient {ing_id} ({ing_name}): OK")
            
            # Verify deletion
            resp = requests.get(f"{BASE_URL}/v2/ingredients/{ing_id}", headers=context["headers"], verify=VERIFY_SSL)
            if resp.status_code == 404:
                print_success("Verified Ingredient deletion")
            else:
                print_error(f"Ingredient still exists after delete: {resp.status_code}")
        else:
            print_error(f"Delete Ingredient failed: {resp.status_code} - {resp.text}")
    else:
        print_error(f"Ingredient {ing_name} not found in context, skipping delete test")

    # Delete Recipe
    rec_name = "Tôm rang me"
    rec_id = context["recipe_ids"].get(rec_name)
    if rec_id:
        resp = requests.delete(f"{BASE_URL}/v2/recipes/{rec_id}", headers=context["headers"], verify=VERIFY_SSL)
        if resp.status_code == 204:
            print_success(f"Delete Recipe {rec_id} ({rec_name}): OK")
            
            # Verify deletion
            resp = requests.get(f"{BASE_URL}/v2/recipes/{rec_id}", headers=context["headers"], verify=VERIFY_SSL)
            if resp.status_code == 404:
                print_success("Verified Recipe deletion")
            else:
                print_error(f"Recipe still exists after delete: {resp.status_code}")
        else:
            print_error(f"Delete Recipe failed: {resp.text}")
