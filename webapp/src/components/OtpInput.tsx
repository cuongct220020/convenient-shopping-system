...
export const OtpInput = ({ length = 6, onComplete, error = false }: OtpInputProps) => {
    ...
    <div className="flex justify-between gap-2 my-8 onPaste={handlePaste}">
      {otp.map((digit, index) => (
        <input
          ...
          className={`w-12 h-14 text-center text-2xl font-bold rounded-lg bg-gray-200 focus:bg-gray-200 focus:border-2 focus:border-gray-600 focus:ring-1 focus:ring-gray-300 focus:cursor-text outline-none transition-all ${digit ? 'text-gray-700' : 'text-transparent'} ${error ? 'border-2 border-red-500' : ''}`}
